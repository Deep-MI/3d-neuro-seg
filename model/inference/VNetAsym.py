...
class Block(nn.Module):
    ...
    def __init__(self, params):
        ...
        kernel_x = (params['kernel_size'][0], 1, 1)
        kernel_y = (1, params['kernel_size'][1], 1)
        kernel_z = (1, 1, params['kernel_size'][2])

        self.conv0 = nn.Sequential(
                        nn.Conv3d(in_channels=params['in_channels'], out_channels=params['out_channels'],
                               kernel_size=kernel_x, padding = (padding_x, padding_y, padding_z),
                               stride=1),
                        ...
                        nn.Conv3d(in_channels=params['out_channels'], out_channels=params['out_channels'],
                                  kernel_size=kernel_y, padding=0,
                                  stride=1),
                        nn.GroupNorm(num_groups=4, num_channels=params['out_channels']),
                        nn.PReLU(),
                        nn.Conv3d(in_channels=params['out_channels'], out_channels=params['out_channels'],
                                  kernel_size=kernel_z, padding=0,
                                  stride=1),
                        nn.GroupNorm(num_groups=4, num_channels=params['out_channels']),
                        nn.PReLU(),

                        nn.Conv3d(in_channels=params['out_channels'], out_channels=params['out_channels'],
                               kernel_size=kernel_x, padding = 0,
                               stride=1),
                        nn.GroupNorm(num_groups=4, num_channels=params['out_channels']),
                        nn.PReLU(),
                        nn.Conv3d(in_channels=params['out_channels'], out_channels=params['out_channels'],
